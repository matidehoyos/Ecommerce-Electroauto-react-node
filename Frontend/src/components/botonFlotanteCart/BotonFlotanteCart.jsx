...
const BotonFlotanteCart = () => {
    ...
    return (
        <div className={style.modalCarrito}>
            {
                carrito.length > 0 ?
                <button className={style.kartButton} style={{ display: modalIsOpen ? 'none' : 'block' }}  onClick={openModal}><span className={style.prod}>Productos seleccionados</span> <IoCartOutline className={style.iconC}/><span className={style.cant}>{carrito.length > 0 ? carrito.length : null}</span></button>
                : null
            }
            <div>
            <Modal
